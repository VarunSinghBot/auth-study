...
export function GoogleLoginButton() {
  ...
  const handleSignIn = async () => {
    setLoading(true);
    // Redirects to Google OAuth, then back to /success?method=oauth
    await signIn("google", { callbackUrl: "/success?method=oauth" });
  };
  ...
  return (
    <button
      ...
      disabled={loading}
      className="w-full flex items-center justify-center gap-3 py-2.5 border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-60"
    >
      {loading ? (
